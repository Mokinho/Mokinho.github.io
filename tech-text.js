/*
 * TechText — vanilla JS port of the React Bits <TechText /> component.
 * Adapted for a multi-line, word-wrapped hero headline:
...
(function () {
  ...
  function TechText(container, options) {
    ...
    const currentSize = () => {
      const el = s.sizeFrom;
      const v = el ? parseFloat(getComputedStyle(el).fontSize) : NaN;
      return Number.isFinite(v) && v > 0 ? v : s.fontSize;
    };
    ...
    const sprite = (size, glyph, stroke) => {
    ...
    // Split segments into lines of { char, color }, greedy-wrapped to maxWidth.
    const wrapLines = (probe, maxWidth) => {
      const chars = [];
      s.segments.forEach(seg => Array.from(seg.text).forEach(char => chars.push({ char, color: seg.color || s.color })));
      const words = [];
      let cur = [];
      const flush = () => { if (cur.length) words.push(cur); cur = []; };
      chars.forEach(c => {
        if (c.char === '\n') { flush(); words.push('\n'); }
        else if (c.char === ' ') flush();
        else cur.push(c);
      });
      flush();
      const lines = [];
      let line = [];
      const text = arr => arr.map(c => c.char).join('');
      words.forEach(w => {
        if (w === '\n') { lines.push(line); line = []; return; }
        const candidate = line.length ? line.concat([{ char: ' ', color: s.color }], w) : w.slice();
        if (line.length && probe.measureText(text(candidate)).width > maxWidth) {
          lines.push(line);
          line = w.slice();
        } else line = candidate;
      });
    ...
    const ensureLayout = () => {
      ...
      const key = [family(), size, width, dpr].join('|');
      if (key === layoutKey && word) return word;
      layoutKey = key;
      const wanted = fontFor(64);
      if (document.fonts && wanted !== requestedFont) {
        requestedFont = wanted;
        document.fonts.load(wanted, s.segments.map(x => x.text).join('')).then(refreshFonts, refreshFonts);
      }

      const probe = scratchCtx;
      setFont(probe, size);
      const lines = wrapLines(probe, Math.max(1, width - s.pad * 2));
      const fm = probe.measureText('Hg');
      const asc = fm.fontBoundingBoxAscent || size * 0.8;
      const desc = fm.fontBoundingBoxDescent || size * 0.2;
      const lh = size * s.lineHeight;

      const nextHeight = Math.ceil(lines.length * lh + s.pad * 2);
      ...
      lines.forEach((lineChars, li) => {
        ...
        lineChars.forEach(({ char, color }) => {
          ...
          bounds.left = Math.min(bounds.left, base.box.x1);
          bounds.right = Math.max(bounds.right, base.box.x2);
          bounds.top = Math.min(bounds.top, base.box.y1);
          bounds.bottom = Math.max(bounds.bottom, base.box.y2);
          ...
          glyphs.push({
            ...
            velocity: { x: 0, y: 0 },
            outline: 0,
            fill: sprite(size, base, false),
            dashes: sprite(size, base, true)
          });
        });
      });
      word = { size, ...bounds };
    ...
    const tick = now => {
      ...
      if (sweeping) {
        // Sweep left↔right while slowly drifting between lines.
        targetX = view.left + (view.right - view.left) * (0.5 - 0.5 * Math.cos(clock * 0.45));
        targetY = view.top + (view.bottom - view.top) * (0.5 - 0.5 * Math.cos(clock * 0.16));
      }
    ...
    const resize = () => {
      ...
      if (nextWidth === width && nextDpr === dpr && word) { layoutKey = ''; wake(); return; }
      width = nextWidth;
      dpr = nextDpr;
